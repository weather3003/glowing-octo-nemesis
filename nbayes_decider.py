...
if __name__ == '__main__':
	...
	class1log = math.log(class1count)
	class2log = math.log(class2count)
	
	for token in queryf_tokens:
		class1log += math.log(float(class1counts.get(token, 1)) / class1count)
		class2log += math.log(float(class2counts.get(token, 1)) / class2count)
		
	result = { 'percent(p1)' : class1log,
						 'percent(p2)' : class2log,
						 'file'    : argv[3] }
